...
object GoogleVsApple {
  ...
  def containsAny(text: String, words: List[String]): Boolean =
    if (words.isEmpty) false
    else if (text contains words.head) true
    else containsAny(text, words.tail)

  lazy val googleTweets: TweetSet = TweetReader.allTweets filter (tweet => containsAny(tweet.text, google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets filter (tweet => containsAny(tweet.text, apple))
